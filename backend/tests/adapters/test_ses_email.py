...
async def test_ses_adapter_passes_both_bodies_to_injected_client() -> None:
    ...
    adapter = SesEmailAdapter(region=REGION, email_from="no-reply@example.com", client=StubClient())
    await adapter.send(
        EmailMessage(to="ada@example.com", subject="Subj", text="t", html="<p>h</p>")
    )
...
async def test_ses_adapter_omits_html_part_when_absent() -> None:
    calls: list[dict[str, Any]] = []
    ...
    adapter = SesEmailAdapter(region=REGION, email_from="no-reply@example.com", client=StubClient())
    await adapter.send(EmailMessage(to="ada@example.com", subject="Subj", text="t"))
    assert "Html" not in calls[0]["Message"]["Body"]


def test_ses_adapter_satisfies_protocol() -> None:
    assert isinstance(
        SesEmailAdapter(region=REGION, email_from="x@y.z", client=object()), EmailAdapter
    )
