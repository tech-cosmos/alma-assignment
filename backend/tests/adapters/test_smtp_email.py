...
async def test_smtp_adapter_delivers_multipart_message(
    ...
    await adapter.send(
        EmailMessage(
            to="ada@example.com", subject="Hello Ada", text="Hi Ada", html="<p>Hi <b>Ada</b></p>"
        )
    )
    ...
    parts = {
        p.get_content_type(): cast(bytes, p.get_payload(decode=True))
        for p in msg.walk()
...
async def test_smtp_adapter_raises_when_server_unreachable() -> None:
    ...
    with pytest.raises(aiosmtplib.SMTPException):
        await adapter.send(EmailMessage(to="ada@example.com", subject="x", text="x"))


def test_build_message_has_text_and_html_alternatives() -> None:
    msg = build_message(
        email_from="a@b.c",
        message=EmailMessage(to="d@e.f", subject="s", text="t", html="<i>h</i>"),
    )
    assert msg.get_content_type() == "multipart/alternative"
    assert [p.get_content_type() for p in msg.iter_parts()] == ["text/plain", "text/html"]


def test_build_message_without_html_is_plain_text() -> None:
    msg = build_message(email_from="a@b.c", message=EmailMessage(to="d@e.f", subject="s", text="t"))
    assert msg.get_content_type() == "text/plain"


def test_smtp_adapter_satisfies_protocol() -> None:
    assert isinstance(SmtpEmailAdapter(host="h", port=1, email_from="x@y.z"), EmailAdapter)
